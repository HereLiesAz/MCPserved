...
/**
 * Connection to the on-device app over a Cloudflare Quick Tunnel — the
 * desktop-side counterpart to the app's one-tap "Start tunnel and send to
 * your AI assistant" button. No Cloudflare account, no API token, no relay
 * to deploy: the tunnel URL is the entire address.
 *
 * Deliberately **not** {@link ./relay-link.js#RelayLink}. A Quick Tunnel
 * (`cloudflared tunnel --url`, see `CloudflareTunnel.kt`) is a bare reverse
 * proxy straight onto the device's own `LocalWsServer` — there is no relay
 * software listening on the other end to broker a room, so dialing
 * `/connect?room=…&role=host` the way `RelayLink` does would just hit
 * whatever `LocalWsServer` does with a request path its `NanoWSD` doesn't
 * expect. This dials the bare tunnel URL directly instead, and runs exactly
 * the same sealed-frame handshake {@link ./app-link.js#AppLink} does over a
 * raw TCP socket — just carried over WebSocket text frames, which is *why*
 * `LocalWsServer` exists as a distinct server from `LocalServer` in the first
 * place (Quick Tunnels proxy HTTP/WebSocket only, never raw TCP; see that
 * class's doc comment).
 *
 * `MCPSERVED_MODE=tunnel` plus `MCPSERVED_HOST=<the https URL the app
 * showed>` select this backend; see index.ts.
 */
export class TunnelLink implements Link {
  ...
  private async ensureConnected(timeoutMs = 15_000): Promise<void> {
    ...
    // The app hands back an https:// URL — Quick Tunnels proxy HTTP/WebSocket,
    // there's no such thing as a plain TCP one — but the WebSocket upgrade
    // needs the ws(s) scheme instead. No path or query beyond that: unlike
    // RelayLink there's no room to name, since nothing is brokering a pairing
    // on the other end.
    const url = this.tunnelUrl.replace(/^http/, "ws");
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ws.terminate();
        reject(new Error(`tunnel connection timed out after ${timeoutMs}ms`));
      }, timeoutMs);
  ...
  private attach(ws: WebSocket): void {
    ...
    ws.on("close", () => {
      ...
      const resolve = this.pending;
      this.pending = null;
      resolve?.({ ok: false, error: "tunnel connection closed" });
    });
