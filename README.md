...
session, say — four opt-in paths exist, off by default and gated behind their
own disclosure in the app: a wider bind for use behind a private mesh; the
phone listening on its own global IPv6 address directly, no NAT and nothing
else in the way; a UPnP port mapping that reaches the phone directly with no
relay and no third party (Wi-Fi only); and a relay that carries the same
sealed-frame protocol without ever being able to decrypt it. See
